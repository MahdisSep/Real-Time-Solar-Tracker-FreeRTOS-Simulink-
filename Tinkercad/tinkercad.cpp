...
void setup() {
  ...
  delay(800);
  lcd.clear();
  lcd.setCursor(0,0); lcd.print("X:---  Y:---");
...
void loop() {
  ...
  double t_norm = ( (double)potansio / 1023.0 ) * 2.0 - 1.0;
  SolarTrackingModel_DW.t = t_norm;

  SolarPanelModel_step();
  ...
  Serial.print("LDR: ");
  Serial.print(ldr1_inner); Serial.print(", ");
  Serial.print(ldr2_inner); Serial.print(", ");
  Serial.print(ldr3_inner); Serial.print(", ");
  Serial.print(ldr4_inner);
  Serial.print(" | Energy: "); Serial.print(SolarTrackingModel_DW.energy, 1);
  ...
  if (millis() - lastPrint > 100) {
    ...
    if (SolarTrackingModel_DW.is_Safe == 0) {
     
      lcd.print("   Danger!   ");
    } else {
      char line0[17];
      snprintf(line0, sizeof(line0), "X:%3d  Y:%3d   ", angleX, angleY);
      lcd.print(line0);
      
      lcd.setCursor(0,1);
     
      int E = (int)round(SolarTrackingModel_DW.energy);
      char line1[17];
   
      snprintf(line1, sizeof(line1), "Energy:%3d", E);
      lcd.print(line1);
    }
  }
